...
class Network:
    ...
    def optimize(self, name, sequence_length, learn_rate, epsilon, c1, c2):
        ...
        with tf.variable_scope(name):
            for t_step in sequence_length:
                alpha_value = self.alpha[t_step,:,:]
                beta_value = self.beta[t_step,:,:]
                action_value = self.action[t_step,:,:]
                advantage_values = self.gae_advantage[t_step,:,:]
                target_values = self.target_value[t_step,:,:]
                observation_values = self.optimization_observation[t_step,:,:]
    ...
    with graph.as_default():
        ...
        while not done:
            ...
            with tf.Session(graph = graph) as session:
                ...
                value, alpha, beta, action = session.run((value, alpha, beta,
                                                        action),
