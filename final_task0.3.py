...
class Training_util:
    ...
    def _create_train_data_step_runs(self, actions, value_estimate):
        ...
        resets = []
        for action, value, env, env_aggregator in zip(actions, value_estimate, self.envs,self.envs_aggregator):
...
class Network:
    def __init__(self, num_units, observation_size, name, action_size,
        ...
        with tf.variable_scope(name):
            ...
            if weights is not None:
                ...
                self.embedding \
                    = tf.nn.relu_layer(self.observation,
                                       self.embedding_weights,
                                       tf.squeeze(self.embedding_bias))
                self.lstm \
                    = CustomBasicLSTMCell(num_units, custom_settings=True,
                                          custom_bias=weights['lstm_bias'],
                                          custom_weights
                                          =weights['lstm_weights'])
                ...
                self.state \
                    = self.lstm.zero_state(batch_size, dtype=tf.float32)

            else:
                self.batch_size = batch_size
                self.observation \
                    = tf.placeholder(tf.float32,
                                     shape=(self.batch_size,observation_size))
                self.embedding_weights \
                    = tf.Variable(initializer((observation_size, num_units)))
                ...
                self.action_alpha_readout_weights \
                    = tf.Variable(initializer(shape=(num_units, action_size)))
                self.action_alpha_readout_bias \
                    = tf.Variable(initializer(shape=(1, action_size)))
                self.action_beta_readout_bias \
                    = tf.Variable(initializer(shape=(1, action_size)))
                self.action_beta_readout_weights \
                    = tf.Variable(initializer(shape=(num_units, action_size)))
                self.value_readout_weights \
                    = tf.Variable(initializer(shape=(num_units, 1)))
                self.value_readout_bias \
                    = tf.Variable(initializer(shape=(1, 1)))
    ...
    def network_parameters(self):
        ...
        return kernel, bias, embedding_weights, embedding_bias, \
               value_readout_weights, value_readout_bias, \
               action_alpha_readout_weights, action_alpha_readout_bias, \
...
for iteration in range(iteration_num):
    ...
    with graph.as_default():
        ...
        if iteration == 0:
            ...
            train_data_network \
                = Network(lstm_unit_num, observation_size,
                          'iteration'+str(iteration)+'train_data_generation',
                          action_size, batch_size_data_creation,
                          utility.weights)
        while not done:
            ...
            with tf.Session(graph = graph) as session:
                ...
                value, alpha, beta, action \
                    = session.run((value, alpha, beta, action),
                                  feed_dict={
                    train_data_network.observation:utility.get_observation()})
                is_done, resets \
                    = utility.create_train_data_step(action, value)
        ...
        if iteration == 0: 
            with tf.Session(graph = graph) as session:
                session.run(tf.global_variables_initializer())
                parameters = session.run(
                    train_data_network.network_parameters())
                weights = {}
                #keys are defined in the hyperparameter list
                for parameter, key in zip(parameters, keys):
                    weights[key] = parameter 
                utility = Training_util(weights, parallel_envs, gae_lambda,
                                        value_gamma, env_name, train_runs,
                                        train_mode, train_run_length)
