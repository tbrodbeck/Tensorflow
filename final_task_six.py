    ...
    def optimize(self, name, sequence_length, epsilon, c1, c2):
        ...
        with tf.variable_scope(name):
            ...
            for t_step in range(sequence_length):
                embedding = tf.nn.relu_layer(self.optimization_observation[t_step, :, :], self.embedding_weights,
                                             tf.squeeze(self.embedding_bias))
                ...
                l_value = tf.square(
                    tf.subtract(val_pred, tf.reshape(self.target_value[t_step, :], (self.batch_size, 1))))
                prob_ratio = tf.divide(prob_product_new, prob_product_old)
                l_clip = tf.minimum(self.gae_advantage[t_step, :] * prob_ratio,
                                    tf.clip_by_value(prob_ratio, 1 - epsilon, 1 + epsilon) * self.gae_advantage[t_step,
                                                                                             :])
    ...
    with graph.as_default():
        ...
        if iteration == 0:
            with tf.Session(graph=graph) as session:
                ...
                utility = Training_util(weights, parallel_envs, gae_lambda, value_gamma, env_name, train_runs,
    ...
    with graph.as_default():
        ...
        for epoch in range(optimization_epochs):
            ...
            for enum, index in enumerate(train_sample_plan):
                ...
                learn_step, summaries = optimizing_network.optimize(
                ...
                with tf.Session(graph=graph) as session:
                    ...
                    _, summaries = session.run([learn_step, summaries], feed_dict=
